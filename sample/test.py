...
if __name__ == "__main__":
    x = {
        "a": {"aa": {"aaa": 9}, "ab": {"aba": 2, "abb": 1}},
        "b": {"aa": {"aaa": 9}, "ab": {"aba": 2, "abb": 1}},
    }
    y = {
        "a": {"ab": {"aba": 2, "abb": 1}},
        "b": {"aa": {"aaa": 9}, "ab": {"aba": 2, "abb": 1}},
    ...
    tm_year, tm_mon, tm_mday, workdayflag, tm_hour = convert_time_stamp(
        str(new_time_stamp)
    )
    date = (datetime.datetime(tm_year, tm_mon, tm_mday)).strftime("%Y%m%d")
    time = (datetime.datetime(tm_year, tm_mon, tm_mday, tm_hour)).strftime("%Y%m%d%H")
    ...
    for date in [
        ...
        past_15_date = (
            datetime.datetime.strptime(date, "%Y%m%d") + datetime.timedelta(days=-15)
        ).strftime("%Y%m%d")
        past_1_date = (
