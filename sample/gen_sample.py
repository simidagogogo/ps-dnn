...
from user_behavior_global import (
    gen_user_feature_last_14day_global,
    gen_user_feature_global_realtime,
)
from user_behavior_local import (
    gen_stat_feature_last_xhours_local,
    gen_action_list_feature_last_xhours_local,
)
...
def gen_local_stat_feas(
    ...
    user_show, user_click, user_ctr = gen_stat_feature_last_xhours_local(
        user_behavior_local_dict_realtime, userid, time_stamp, hours
    )
    ad_show, ad_click, ad_ctr = gen_stat_feature_last_xhours_local(
        ad_behavior_local_dict_realtime, adgroup_id, time_stamp, hours
    )
...
def gen_local_behavior_feas(
    ...
    non_click_aid_dict, click_aid_dict, last_click = (
        gen_action_list_feature_last_xhours_local(
            user_behavior_local_dict_realtime, userid, time_stamp, hours
        )
    )
...
if __name__ == "__main__":
    ...
    load_behavior_fea_dict(
        "final_data/user_behavior_1day_global_realtime",
        user_behavior_1day_global_realtime,
    )
    print(
        gen_user_feature_global_realtime(
            user_behavior_1day_global_realtime, "305254", "1494160210", "pv", "cate"
        )
    )
    print(
        gen_user_feature_global_realtime(
            user_behavior_1day_global_realtime, "305254", "1494155252", "pv", "brand"
        )
    )
    print(
        gen_user_feature_global_realtime(
            user_behavior_1day_global_realtime, "305254", "1494312046", "buy", "cate"
        )
    )
    print(
        gen_user_feature_global_realtime(
            user_behavior_1day_global_realtime, "305254", "1494312047", "buy", "brand"
        )
    )

    load_behavior_fea_dict(
        "final_data/user_behavior_last_14day_global",
        user_behavior_dict_last_14day_global,
    )
    # check if user_behavior_dict_last_14day_global is ok
    print(
        gen_user_feature_last_14day_global(
            user_behavior_dict_last_14day_global, "558157", "20170513", "pv", "cate"
        )
    )
    print(
        gen_user_feature_last_14day_global(
            user_behavior_dict_last_14day_global, "558157", "20170513", "pv", "brand"
        )
    )

    print("Load user/ad local behavior feature")
    load_behavior_fea_dict(
        "final_data/user_behavior_local_dict_realtime",
        user_behavior_local_dict_realtime,
    )
    print(
        gen_stat_feature_last_xhours_local(
            user_behavior_local_dict_realtime, "581738", "1494137644", 24
        )
    )
    print(
        gen_action_list_feature_last_xhours_local(
    ...
    load_behavior_fea_dict(
        "final_data/ad_behavior_local_dict_realtime", ad_behavior_local_dict_realtime
    )
    print(
        gen_stat_feature_last_xhours_local(
            ad_behavior_local_dict_realtime, "1", "1494137645", 24
        )
    )

    # raw_sample 格式:user,time_stamp,adgroup_id,pid,nonclk,clk
    with open("uncompress/raw_sample.csv", "r") as input_data, open(
        "final_data/train_data", "w+"
    ) as train_data, open("final_data/test_data", "w+") as test_data:
        # 第一行为格式说明，可以跳过
        ...
        while line:
            ...
            for action_type in ["pv", "fav", "cart", "buy"]:
                for dim in ["cate", "brand"]:
                    user_feature_last_14day = gen_user_feature_last_14day_global(
                        user_behavior_dict_last_14day_global,
                        userid,
                        date,
                        action_type,
            ...
            for action_type in ["pv", "fav", "cart", "buy"]:
                for dim in ["cate", "brand"]:
                    latest_n_feas, latest_1_fea = gen_user_feature_global_realtime(
                        user_behavior_1day_global_realtime,
                        userid,
                        time_stamp,
                        action_type,
