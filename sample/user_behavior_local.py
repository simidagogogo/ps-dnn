...
def add_behavior_local_dict_realtime(
    behavior_local_dict_realtime,
    id,
    object_id,
    date,
    tm_hour,
...
def gen_local_behavior_dict_realtime(
    user_behavior_local_dict_realtime, ad_behavior_local_dict_realtime
):
    count = 0
    with open("uncompress/raw_sample.csv", "r") as input_data:
        ...
        while line:
            ...
            add_behavior_local_dict_realtime(
    ...
    for userid in user_behavior_local_dict_realtime:
        for date in user_behavior_local_dict_realtime[userid]:
            for tm_hour in user_behavior_local_dict_realtime[userid][date]:
                (
                    user_behavior_local_dict_realtime[userid][date][tm_hour][
                        "behaviors"
                    ]
                ).sort()
    ...
    for adgroup_id in ad_behavior_local_dict_realtime:
        for date in ad_behavior_local_dict_realtime[adgroup_id]:
            for tm_hour in ad_behavior_local_dict_realtime[adgroup_id][date]:
                (
                    ad_behavior_local_dict_realtime[adgroup_id][date][tm_hour][
                        "behaviors"
                    ]
                ).sort()
...
def gen_stat_feature_last_xhours_local(
    behavior_local_dict_realtime, id, time_stamp, hours
):
    total_show = 0
    total_clk = 0
    ...
    for i in range(hours - 1, 0, -1):
        ...
        if (
            id in behavior_local_dict_realtime
            and new_date in behavior_local_dict_realtime[id]
            and new_tm_hour in behavior_local_dict_realtime[id][new_date]
        ):
            total_show += behavior_local_dict_realtime[id][new_date][new_tm_hour][
                "show"
            ]
            total_clk += behavior_local_dict_realtime[id][new_date][new_tm_hour][
                "click"
            ]
            # print("other hour")
    ...
    if (
        id in behavior_local_dict_realtime
        and date in behavior_local_dict_realtime[id]
        and tm_hour in behavior_local_dict_realtime[id][date]
    ):
        show_this_hour, clk_this_hour = action_stat_from_seqs(
            behavior_local_dict_realtime[id][date][tm_hour]["behaviors"], time_stamp
        )
...
def gen_action_list_feature_last_xhours_local(
    behavior_local_dict_realtime, id, time_stamp, hours
):
    ...
    for i in range(hours - 1, 0, -1):
        ...
        if (
            ...
            user_action_dict_1hour = behavior_local_dict_realtime[id][new_date][
                new_tm_hour
            ]
    ...
    if (
        ...
        last_click_tmp = gen_action_list_from_seqs(
            user_action_dict, time_stamp, shows, clks
        )
        if last_click_tmp != "":
            last_click = last_click_tmp
        # print("this hour")
...
if __name__ == "__main__":
    ...
    gen_local_behavior_dict_realtime(
        user_behavior_local_dict_realtime, ad_behavior_local_dict_realtime
    )
