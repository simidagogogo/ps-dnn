...
def feature_extract_stage1(raw_sample_file, intermediate_sample_file):
    ...
    with open(raw_sample_file, "r") as f, open(intermediate_sample_file, "w") as g:
        ...
        while raw_sample:
            ...
            libfe.fea_extract_stage1(
                ctypes.c_char_p(fea_conf_file),
                ctypes.c_char_p(raw_sample),
                ctypes.byref(buf),
                ctypes.byref(c_buflen),
            )
            if c_buflen.value > 0:
            ...
            if count % 100000 == 0:
                print(
                    ...
                    ":feature_extract_stage1 ",
                    count,
                    " samples processed",
                )


def feature_extract_stage2(
    intermediate_sample_file, sparse_dict_index_file, final_sample_file
):
    libfe = ctypes.cdll.LoadLibrary("./libfe.so")
    length = 81920
    buf = ctypes.create_string_buffer(length)
    ...
    with open(intermediate_sample_file, "r") as f, open(final_sample_file, "w") as g:
        ...
        while intermediate_sample:
            ...
            libfe.fea_extract_stage2(
                ctypes.c_char_p(fea_conf_file),
                ctypes.c_char_p(sparse_dict_index_file),
                ctypes.c_char_p(intermediate_sample),
                ctypes.byref(buf),
                ctypes.byref(c_buflen),
            )
            if c_buflen.value > 0:
...
if __name__ == "__main__":
    ...
    gen_fea_dict(
        intermediate_sample_train_file,
        raw_sparse_dict_file,
        sparse_dict_index_file,
        min_fea_freq,
    )

    # 3. Gen final_sample for train&test
    feature_extract_stage2(
        intermediate_sample_train_file, sparse_dict_index_file, final_sample_train_file
    )
    feature_extract_stage2(
        intermediate_sample_test_file, sparse_dict_index_file, final_sample_test_file
    )
